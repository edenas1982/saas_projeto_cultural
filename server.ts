...
// Handle Google Cloud JSON Credentials for Text Generation (Vertex AI)
if (process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
  try {
    const tmpCreds = path.join(os.tmpdir(), "google-creds.json");
    fs.writeFileSync(tmpCreds, process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON);
    process.env.GOOGLE_APPLICATION_CREDENTIALS = tmpCreds;
  } catch (error) {
    console.error("Erro ao escrever GOOGLE_APPLICATION_CREDENTIALS_JSON", error);
  }
}
...
import { InviteManagementService } from "./src/services/InviteManagementService.js";
import { PhotoInviteService } from "./src/services/PhotoInviteService.js";
...
async function startServer() {
    ...
    try {
      ...
      if (!perguntas_ids || perguntas_ids.length === 0) {
        ...
        if (allPerguntas && allPerguntas.length > 0) {
          ...
          // Se todas as perguntas já estiverem atribuídas, significa que temos que fazer um "rateio"
          // das perguntas não respondidas entre os familiares ativos + o novo!
          if (finalPerguntasIds.length === 0 && convitesAtivos && convitesAtivos.length > 0) {
             const allUnanswered = allPerguntas.map((p: any) => p.id).filter((id: string) => !respondidasIds.includes(id));
             
             // O número de familiares no rateio = ativos + o nosso novo
             const numActive = convitesAtivos.length + 1;
             const base = Math.floor(allUnanswered.length / numActive);
             const remainder = allUnanswered.length % numActive;
             
             let startIndex = 0;
             // Atualizar os antigos primeiro no banco de dados!
             for (let i = 0; i < convitesAtivos.length; i++) {
                 const countForThis = base + (i < remainder ? 1 : 0);
                 const assignThese = allUnanswered.slice(startIndex, startIndex + countForThis);
                 startIndex += countForThis;
                 // Atualizar o invite no banco
                 await supabase.from("question_invites").update({ perguntas_ids: assignThese }).eq("id", convitesAtivos[i].id);
             }
             
             // Por fim, as perguntas do novo familiar
             finalPerguntasIds = allUnanswered.slice(startIndex, allUnanswered.length);
          } else if (finalPerguntasIds.length === 0) {
             // Caso raríssimo (não há perguntas não respondidas e nenhum convite ativo (?)
             finalPerguntasIds = allPerguntas.map((p: any) => p.id).filter((id: string) => !respondidasIds.includes(id));
      ...
      let familyAccountIdToUse = memorial.family_account_id;
      
      if (!familyAccountIdToUse && user?.id) {
         // Create or find a family account for this auth user to satisfy the foreign key constraint
         const { data: famAccount } = await supabase.from('family_accounts').select('id').eq('auth_user_id', user.id).maybeSingle();
         if (famAccount) {
            familyAccountIdToUse = famAccount.id;
         } else {
            const { data: newFam, error: famErr } = await supabase.from('family_accounts').insert({
               auth_user_id: user.id,
               nome: 'Organização SaaS',
               email: `${user.id}@saas.com`
            }).select('id').single();
            if (!famErr && newFam) {
               familyAccountIdToUse = newFam.id;
            } else {
               familyAccountIdToUse = user.id; // Fallback bound to fail, but keeps original behavior
            }
         }
      }
      ...
      const { data: inviteArray, error: inviteError } = await supabase
        ...
        .insert({
          memorial_id,
          family_account_id: familyAccountIdToUse,
          destinatario_nome,
          destinatario_tel,
      ...
      // Verificações parciais / recontagem global de invites
      try {
        ...
        await supabase.from('memoriais').update({ 
          ...
          // Atualiza status se não estiver já concluído/gerado
        }).eq('id', memorial_id);

      } catch (e) {
        console.error("Erro recálculo invites:", e);
      }
  ...
  // Rota Privada: Deletar convites ativos de um memorial
  app.delete("/api/invites/memorial/:memorialId", authenticateToken, async (req, res) => {
    try {
      const { memorialId } = req.params;
  ...
  // NOVAS ROTAS - GESTAO DE PARTICIPANTES (MODULO 5)
  const auditServiceInstance = new AuditService();
  const inviteManagementService = new InviteManagementService(auditServiceInstance);
  const photoInviteService = new PhotoInviteService(auditServiceInstance);

  app.put("/api/invites/:inviteId", authenticateToken, async (req, res) => {
    try {
      const { inviteId } = req.params;
      ...
      const result = await inviteManagementService.updateInvite(supabase, inviteId, { nome, telefone }, operatorId);
      return res.json({ success: true, data: result });
    } catch (error: any) {
      return res.status(error.status || 500).json({ error: error.message || 'Erro' });
    }
  });

  app.post("/api/invites/:inviteId/reenable", authenticateToken, async (req, res) => {
     try {
       const { inviteId } = req.params;
       const operatorId = (req as any).user.id || "system";
       let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL;
       ...
       const result = await inviteManagementService.reenableInvite(supabase, inviteId, operatorId);
       return res.json({ success: true, data: result });
     } catch (error: any) {
        return res.status(error.status || 500).json({ error: error.message || 'Erro' });
     }
  ...
  app.post("/api/invites/:inviteId/revoke", authenticateToken, async (req, res) => {
     try {
       const { inviteId } = req.params;
       const operatorId = (req as any).user.id || "system";
       let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL;
       ...
       const result = await inviteManagementService.revokeReenabled(supabase, inviteId, operatorId);
       return res.json({ success: true, data: result });
      } catch (error: any) {
         return res.status(error.status || 500).json({ error: error.message || 'Erro' });
      }
  ...
  app.post("/api/invites/:inviteId/append", authenticateToken, async (req, res) => {
     try {
       const { inviteId } = req.params;
       ...
       const result = await inviteManagementService.appendQuestionsToInvite(supabase, inviteId, questions || [], operatorId);
       return res.json({ success: true, data: result });
     } catch (error: any) {
        return res.status(error.status || 500).json({ error: error.message || 'Erro' });
     }
  ...
  app.delete("/api/invites/:inviteId", authenticateToken, async (req, res) => {
     try {
       const { inviteId } = req.params;
       const operatorId = (req as any).user.id || "system";
       let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL;
       ...
       const result = await inviteManagementService.deleteInvite(supabase, inviteId, operatorId);
  ...
  app.get("/api/public/invite/:token", async (req, res) => {
    ...
    try {
      ...
      const invite = inviteArray?.[0];
      if (inviteError || !invite) {
        console.error('Invite not found:', token, inviteError);
      ...
      if (new Date(invite.expira_em) < new Date() || invite.status === 'concluido' || invite.status === 'expirado') {
        console.error('Invite expired or completed:', invite.id, invite.status, invite.expira_em);
      ...
      // Load existing responses if they exist for this memorial & questions
      let existing_respostas: any[] = [];
      if (invite.perguntas_ids && invite.perguntas_ids.length > 0) {
          const { data: respostasDb } = await supabase
              .from('respostas')
              .select('pergunta_id, resposta')
              .eq('memorial_id', memorial.id)
              .in('pergunta_id', invite.perguntas_ids);
          existing_respostas = respostasDb || [];
      }

      return res.json({ success: true, invite, memorial, perguntas, existing_respostas });
    } catch (error: any) {
      console.error(error);
  ...
  app.post("/api/public/invite/:token/respostas", async (req, res) => {
    try {
      ...
      // Verificações finais e atualização do status: Respostas Parciais vs Recebidas
      ...
      // 3. Verifica total de questões já respondidas globalmente
      ...
      // 4. Determinar status correto
      const _totalCriados = totalCriados || 0;
      const _totalConcluidos = totalConcluidos || 0;
      const novoStatus = _totalConcluidos >= _totalCriados && _totalCriados > 0
        ? 'respostas_recebidas'
        : 'respostas_parciais';

      // 5. Atualizar memorial
  ...
  // ==========================================
  // ROTAS DO FOTOLINK (MODULO 3)
  // ==========================================

  // Rota Privada: Criar/Buscar FotoLink para memorial
  app.post("/api/memoriais/:id/photo-invite/create", authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      const operatorId = (req as any).user.id || "system";
      let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL;
      ...
      const invite = await photoInviteService.createInvite(supabase, id, operatorId);
      return res.json({ success: true, invite });
    } catch (error: any) {
      return res.status(error.status || 500).json({ error: error.message || 'Erro ao criar FotoLink' });
    }
  });

  // Rota Privada: Listar fotos (perfil + galeria) do memorial
  app.get("/api/memoriais/:id/fotos", authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL;
      const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
      ...
      return res.json({
        success: true,
        perfil: memorial?.foto_url || null,
        galeria: midias || []
      });
    } catch (error: any) {
      return res.status(500).json({ error: error.message || 'Erro ao carregar fotos' });
    }
  });

  // Rota Pública: Carregar dados do FotoLink por token
  app.get("/api/public/fotos/:token", async (req, res) => {
    res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
    res.setHeader('Pragma', 'no-cache');
    res.setHeader('Expires', '0');
    try {
      ...
      const data = await photoInviteService.getPublicData(supabase, token);
      return res.json({ success: true, ...data });
    } catch (error: any) {
      return res.status(error.status || 500).json({ error: error.message || 'Erro ao carregar dados' });
    }
  });

  // Rota Pública: Validar limite de upload de fotos do plano por token
  app.post("/api/public/fotos/:token/upload/validate", async (req, res) => {
    try {
      const { token } = req.params;
      const { tipo } = req.body;
      if (tipo !== 'perfil' && tipo !== 'galeria') {
      ...
      const result = await photoInviteService.validateLimits(supabase, token, tipo);
      return res.json({ success: true, ...result });
    } catch (error: any) {
      return res.status(error.status || 500).json({ error: error.message || 'Erro ao validar limites' });
    }
  });

  // Rota Pública: Registrar upload de foto (perfil ou galeria) após o upload no Storage
  app.post("/api/public/fotos/:token/upload", async (req, res) => {
    try {
      const { token } = req.params;
      const { tipo, url } = req.body;
      if (!tipo || !url) {
        return res.status(400).json({ error: 'Dados incompletos para registro do upload' });
      }
      ...
      // 1. Valida se o link está ativo e se a conta do plano permite
      const limitCheck = await photoInviteService.validateLimits(supabase, token, tipo);
      if (!limitCheck.allowed) {
        return res.status(400).json({ error: `Limite do plano atingido para o tipo ${tipo}` });
      }

      // 2. Busca dados do convite para pegar o memorial_id
      ...
      if (tipo === 'perfil') {
        // Grava na coluna foto_url do memorial
      ...
      return res.status(error.status || 500).json({ error: error.message || 'Erro ao registrar upload' });
    }
  });

  // Rota Pública: Enviar notificação de fotos recebidas
  app.post("/api/public/fotos/:token/notify", async (req, res) => {
    try {
      const { token } = req.params;
      ...
      await photoInviteService.notifyPhotosReceived(supabase, token, numCount);
      return res.json({ success: true });
    } catch (error: any) {
      return res.status(error.status || 500).json({ error: error.message || 'Erro ao processar notificação' });
    }
  });
...
`;
  ...
  app.post("/api/narrativas/audio", async (req, res) => {
    ...
    try {
      ...
      if (!googleApiKey) {
        return res.status(500).json({ error: "Credenciais do Google Cloud para áudio não configuradas (Insira GOOGLE_TTS_API_KEY no painel de Segredos)" });
      }
