...
const aiLimiter = rateLimit({
  ...
  message: { error: 'Limite de gerações por hora atingido, tente novamente mais tarde.' }
});

app.use('/api/', apiLimiter);

// HELPER DE AUTENTICAÇÃO E OWNERSHIP
async function requireAuthAndOwnership(req: express.Request, res: express.Response, memorial_id: string, supabase: any) {
  ...
  if (memorial.user_id !== user.id) {
    res.status(403).json({ error: 'Sem permissão para modificar este memorial' });
    return null;
  }
...
app.post("/api/narrativas/generate", aiLimiter, async (req, res) => {
  try {
    ...
    const { memorial_id, respostas, perfil_tom, densidade, total_perguntas, contexto_adicional, texto_base } = parseResult.data;
...
Gere a biografia em quatro blocos claramente separados usando exatamente estes marcadores. IDENTIDADE_INICIO e IDENTIDADE_FIM para o bloco da gaveta Identidade. JORNADA_INICIO e JORNADA_FIM para o bloco da gaveta Jornada. ESSENCIA_INICIO e ESSENCIA_FIM para o bloco da gaveta Essencia. LEGADO_INICIO e LEGADO_FIM para o bloco da gaveta Legado.`;
    ...
    if (!user) return; // Response já enviada
      ...
      const { count: attemptCount, error: countError } = await supabase
        ...
        .in('motivo_geracao', ['primeira geracao', 'regeneracao parcial', 'regeneracao completa', 'mudanca de tom']);
      ...
      if ((attemptCount || 0) >= 5) {
        return res.status(429).json({ error: 'Você atingiu o limite de 5 gerações para este memorial. Entre em contato com o suporte para adquirir mais créditos.' });
      }
...
`;
    ...
    const conteudoCompleto = message.content[0].type === 'text' ? message.content[0].text : '';
    ...
    const identidadeRenderizada = extrairBloco(conteudoCompleto, 'IDENTIDADE_INICIO', 'IDENTIDADE_FIM');
    const jornadaRenderizada = extrairBloco(conteudoCompleto, 'JORNADA_INICIO', 'JORNADA_FIM');
    const essenciaRenderizada = extrairBloco(conteudoCompleto, 'ESSENCIA_INICIO', 'ESSENCIA_FIM');
    const legadoRenderizado = extrairBloco(conteudoCompleto, 'LEGADO_INICIO', 'LEGADO_FIM');

    const conteudoLimpo = [identidadeRenderizada, jornadaRenderizada, essenciaRenderizada, legadoRenderizado]
    ...
    const { data: narrativa, error: narrativaError } = await supabase
      ...
      .insert({
        memorial_id,
        template: 'visual',
        conteudo_completo: conteudoLimpo || conteudoCompleto.replace(/(IDENTIDADE_INICIO|IDENTIDADE_FIM|JORNADA_INICIO|JORNADA_FIM|ESSENCIA_INICIO|ESSENCIA_FIM|LEGADO_INICIO|LEGADO_FIM)/g, '').replace(/\n{3,}/g, '\n\n').trim(),
    ...
    return res.status(500).json({ error: error?.message || 'Erro interno na geracao da narrativa' });
  }
});
...
// Rota para ajustar biografia pontualmente
  app.post("/api/narrativas/adjust", aiLimiter, async (req, res) => {
    try {
      ...
      console.log('Adjust Request parsed data:', { memorial_id, narrativa_id, instrucao });

      if (!memorial_id || !narrativa_id || !instrucao || typeof instrucao !== 'string' || instrucao.trim() === '') {
        return res.status(400).json({ error: 'Dados insuficientes para ajuste (instrução não pode ser vazia)' });
      }
      ...
      if (dbNarrativa.modo_conteudo === 'editado_pelo_usuario') {
        ...
        if (respostasData && respostasData.length > 0) {
           textoRespostas = respostasData.map(r => `Pergunta: ${r.pergunta_texto_snapshot}\nResposta: ${r.resposta}`).join('\n\n');
        } else {
      ...
      const conteudoAjustado = message.content[0].type === 'text' ? message.content[0].text : '';
      
      const tokens_input = message.usage?.input_tokens || 0;
      const tokens_output = message.usage?.output_tokens || 0;
      const custo_estimado = (tokens_input * (3.00 / 1000000)) + (tokens_output * (15.00 / 1000000)); // Sonnet 3.5 aprox costs
      ...
      return res.status(500).json({ error: error?.message || 'Erro interno no ajuste da narrativa' });
    }
  });
...
const AudioSchema = z.object({
  ...
  app.post("/api/narrativas/audio", aiLimiter, async (req, res) => {
    try {
      ...
      if (!googleCreds) {
        return res.status(500).json({ error: "Credenciais do Google Cloud não configuradas (GOOGLE_APPLICATION_CREDENTIALS_JSON)" });
      }
      ...
      for(const chunk of chunks) {
        ...
        if (response.audioContent) {
          audioBuffers.push(response.audioContent as Uint8Array);
        }
      }
      ...
      return res.status(500).json({ error: error?.message || 'Erro interno na geração de áudio' });
    }
  });
