...
export function Saas_NewLinkModal({ isOpen, onClose, onGenerate, memorial, initialTab, onRequestPlanChange, shareMode = false }: NewLinkModalProps) {
  ...
  // Estados de edição inline dos cards de convite na tela de sucesso
  const [editingInviteId, setEditingInviteId] = useState<string | null>(null);
  const [editInviteNome, setEditInviteNome] = useState('');
  const [editInviteTel, setEditInviteTel] = useState('');
  const [isSavingInviteEdit, setIsSavingInviteEdit] = useState(false);
      ...
      const fetchExistingInvites = async () => {
        ...
        try {
          ...
          if (data && data.length > 0) {
            ...
            const existingContatos = sortedData.map((inv: any) => ({
              nome: inv.destinatario_nome || '',
              whatsapp: inv.destinatario_tel || ''
            }));
  ...
  const markInviteAsSent = async (id: string) => {
    try {
      const { data: sessionData } = await supabase.auth.getSession();
      const token = sessionData?.session?.access_token;
      if (token) {
        ...
        setGeneratedInvites(prev => prev.map(inv => inv.id === id ? { ...inv, status: 'enviado' } : inv));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const copyToClipboard = async (id: string, link: string) => {
    ...
    await markInviteAsSent(id);
    await updateMemorialStatusAguardando();
  };

  const sendWhatsApp = async (id: string, nome: string, whatsapp: string, link: string) => {
   ...
   * @description Atualiza pontualmente nome e telefone de um convite já gerado,
   *              diretamente na tela de links, sem recriar o memorial.
   * @entity question_invites
   * @rules Requer JWT válido; não altera perguntas_ids nem status do convite.
   * @audit false
   */
  const handleEditInviteSave = async (inviteId: string) => {
    if (!editInviteNome.trim() || !editInviteTel.trim()) {
      setEditInviteError('Nome e WhatsApp são obrigatórios.');
      return;
    }
    const { data: sessionData } = await supabase.auth.getSession();
    const token = sessionData?.session?.access_token;
    if (!token) {
      setEditInviteError('Erro de autenticação.');
      return;
    }
    setIsSavingInviteEdit(true);
    setEditInviteError('');
    try {
      const res = await fetch(`/api/invites/${inviteId}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome: editInviteNome.trim(), telefone: editInviteTel.trim() })
      });
      if (!res.ok) {
        const er = await res.json();
        setEditInviteError(er.error || 'Erro ao salvar. Tente novamente.');
        return;
      }
      // Atualiza o estado local sem fechar o modal
      setGeneratedInvites(prev =>
        prev.map(inv =>
          inv.id === inviteId
            ? { ...inv, nome: editInviteNome.trim(), whatsapp: editInviteTel.trim() }
            : inv
        )
      );
      setEditingInviteId(null);
    } catch (e: any) {
      setEditInviteError(e.message || 'Erro inesperado.');
    } finally {
      setIsSavingInviteEdit(false);
    }
  };
  ...
  const handleSubmitShareMode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(`/api/invites/memorial/${memorial.id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const resInvites = await fetch(`/api/invites/memorial/${memorial.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const jsonInvites = await resInvites.json();
      const convitesAtivos = (jsonInvites.invites || []).filter((c: any) => ['pendente', 'enviado', 'acessado'].includes(c.status));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 my-4">
        ...
        {isLoadingInvites ? (
          ...
          <div className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div key={invite.id} className="border border-slate-200 rounded-xl overflow-hidden">
                  <div className="bg-slate-50 px-4 py-2.5 flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      ...
                      {(invite.status === 'enviado' || invite.status === 'acessado' || invite.status === 'concluido') && (
                        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full flex items-center gap-1 border border-emerald-100">
                  ...
                  {editingInviteId === invite.id ? (
                    <div className="px-4 py-3 flex flex-col gap-2 bg-slate-50 border-t border-slate-100">
                      <div className="grid grid-cols-2 gap-2">
                        <div className="relative">
                          <User className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-2.5" />
                          <input
                            type="text"
                            value={editInviteNome}
                            onChange={e => setEditInviteNome(e.target.value)}
                            placeholder="Nome"
                            className="w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 bg-white"
                          />
                        </div>
                        <div className="relative">
                          ...
                          <input
                            type="tel"
                            value={editInviteTel}
                            onChange={e => setEditInviteTel(e.target.value)}
                      ...
                      {editInviteError && (
                        <p className="text-[10px] text-rose-500 font-medium">{editInviteError}</p>
                      )}
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => { setEditingInviteId(null); setEditInviteError(''); }}
                          disabled={isSavingInviteEdit}
                          className="flex-1 py-1.5 text-xs font-semibold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-100 transition-colors disabled:opacity-50"
                        >
                          Cancelar
                        </button>
                        <button
                          type="button"
                          onClick={() => handleEditInviteSave(invite.id)}
                          disabled={isSavingInviteEdit}
                          className="flex-1 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors flex items-center justify-center gap-1 disabled:opacity-70"
                        >
                          {isSavingInviteEdit ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
                          Salvar
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="px-4 py-2.5 flex gap-2">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setEditInviteNome(invite.nome);
                          setEditInviteTel(invite.whatsapp);
                          setEditInviteError('');
                        }}
                        className="border border-slate-200 hover:bg-slate-50 text-slate-500 hover:text-indigo-600 hover:border-indigo-300 transition-all py-1.5 px-3 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 text-slate-600 hover:text-indigo-700 transition-all py-1.5 px-3 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5"
                      >
                        {copiedId === invite.id
                      ...
                      <button
                        type="button"
                        onClick={() => sendWhatsApp(invite.id, invite.nome, invite.whatsapp, invite.link)}
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white transition-colors py-1.5 px-3 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5"
                      >
            ...
            <div className="flex gap-3 mt-5">
              ...
              >
                {shareMode ? 'Editar Contatos / Reenviar' : 'Editar Dados do Memorial'}
              </button>
              <button
                type="button"
                onClick={handleCloseAndReset}
                className={`text-white text-sm font-semibold py-3 rounded-xl transition-colors flex-1 bg-slate-900 hover:bg-slate-800`}
              >
                Concluir
              </button>
            </div>
