...
export default function PublicInvite() {
  ...
  useEffect(() => {
    async function loadInvite() {
      try {
        ...
        if (data.existing_respostas && Array.isArray(data.existing_respostas)) {
            const loaded: Record<string, string> = {};
            data.existing_respostas.forEach((r: any) => {
                if (r.resposta) loaded[r.pergunta_id] = r.resposta;
            });
            setRespostas(loaded);
            
            const sortedPerguntas = [...data.perguntas].sort((a, b) => {
               const aHasRes = !!loaded[a.id];
               const bHasRes = !!loaded[b.id];
  ...
  const handleTestFill = () => {
    const answerMap: Record<string, string> = {
      'ide_01': "Antônio Alves da Silva. Nasceu em uma fazenda pequena no interior de Minas Gerais, perto de Poços de Caldas.",
      'ide_02': "Nasceu em 1945. Cresceu na roça, sem energia elétrica no começo, brincando no quintal de terra batida entre os pés de café.",
      'ide_03': "Filho do 'Seu' João e da Dona Maria, lavradores que vieram de carroça do interior de São Paulo em busca de terra boa.",
      'ide_04': "A casinha com varanda que ele comprou com muito suor em Campinas, interior de São Paulo.",
      'ide_05': "Chamavam ele de 'Tonho Ventania' porque não parava quieto, sempre correndo para ajudar o pai na roça.",
      'jor_01': "Foi marceneiro a vida inteira. Tinha as mãos grossas, mas um cuidado de artista para fazer móveis perfeitos.",
      'jor_02': "Casou-se com a Dona Helena, o amor da vida dele. Ficaram juntos por 50 anos e tiveram 4 filhos e 9 netos.",
      'jor_03': "Morou em Poços de Caldas, depois mudou-se para Campinas onde passou o resto da vida.",
      'jor_04': "Quando teve coragem de largar o emprego na fábrica para abrir a própria marcenaria na garagem de casa. Passou aperto, mas venceu.",
      'jor_05': "Não era de falar muito. O jeito dele amar era consertar as coisas quebradas na casa dos filhos e fazer brinquedos de madeira para os netos.",
      'ess_01': "Cheiro de pó de serra misturado com café forte passado na hora.",
      'ess_02': "Sentar na cadeira de fio na calçada no fim da tarde, escutando rádio AM bem baixinho.",
      'ess_03': "Guardava parafusos e pregos velhos em potes de margarina dizendo 'um dia vai servir' (e sempre servia).",
      'ess_04': "Macarronada com frango assado com a casa cheia, seguida do cochilo sagrado no sofá com o jogo de futebol na TV.",
      'ess_05': "Um homem de poucas palavras, mas de um coração gigante, trabalhador incansável e o grande porto seguro da nossa família.",
      'leg_01': "'Deus não dá um fardo maior do que a gente tem força para carregar.'",
      'leg_02': "Que o trabalho honesto é a nossa maior riqueza e que a família deve sempre sentar unida na mesa, não importam as brigas.",
      'leg_03': "Era o conselheiro quieto. Ele escutava todo mundo e só falava quando era para dar uma palavra de sabedoria.",
      'leg_04': "A vez que ele tentou fazer um bolo surpresa para a Helena e confundiu o sal com o açúcar. O bolo ficou horrível, mas comemos tudo de tanto orgulho dele.",
      'leg_05': "'Não chorem porque fui embora, fiquem felizes pelas coisas lindas que vivemos. Cuidem bem da mãe de vocês.'"
    };
    ...
    perguntas.forEach((p) => {
      if (answerMap[p.id]) {
        newRespostas[p.id] = answerMap[p.id];
      }
    });
          ...
          {perguntas.map((p, idx) => (
            <div key={p.id} className="bg-white rounded-2xl shadow-sm border p-6 md:p-8 transition-shadow focus-within:ring-2 focus-within:ring-emerald-500 focus-within:ring-opacity-50">
              <label htmlFor={`pergunta-${p.id}`} className="block">
                ...
                <span className="text-xl font-medium text-gray-900 block mb-2">{p.ordem}. {p.texto_pergunta}</span>
