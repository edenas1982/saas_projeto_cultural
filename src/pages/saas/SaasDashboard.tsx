...
import { Users, Link as LinkIcon, TrendingUp, AlertCircle, Copy, CheckCircle2, Search, Filter, Edit, Image as ImageIcon, Share2, Loader2, MoreVertical, MessageCircle, Send, Wand2, RefreshCw, QrCode, Download, X, Upload, Key, Crown, Briefcase, Bell, EyeOff, Eye } from 'lucide-react';
...
export default function SaasDashboard() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('memorial_changes')
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'memoriais' },
        (payload) => {
          const updatedMemorial = payload.new;
          setMemoriais(prevMemoriais => {
            const oldMemorial = prevMemoriais.find(m => m.id === updatedMemorial.id);
            if (oldMemorial) {
              const merged = { ...oldMemorial, ...updatedMemorial };
              
              if (
                 (oldMemorial.status_memorial !== updatedMemorial.status_memorial && (updatedMemorial.status_memorial === 'respostas_recebidas' || updatedMemorial.status_memorial === 'Respondido')) ||
                 (oldMemorial.status !== updatedMemorial.status && updatedMemorial.status === 'respostas_recebidas')
              ) {
                 showToast(`🔔 A família de ${updatedMemorial.nome_homenageado || 'um homenageado'} acabou de enviar as respostas!`);
              }
              
              return prevMemoriais.map(m => m.id === updatedMemorial.id ? merged : m);
            }
            return prevMemoriais;
          });
        }
      )
      .subscribe();
  ...
  const getStatusStyle = (status: string) => {
    switch(status) {
      ...
      case 'respostas_recebidas': return 'bg-blue-100 text-blue-700 border-blue-200';
  ...
  const getStatusLabel = (status: string) => {
    switch(status) {
      ...
      case 'em_progresso': 
      case 'aguardando': return 'Enviado / Aguardando Resposta';
      case 'respostas_recebidas': return 'Retorno / Perguntas Respondidas';
  ...
  return (
    <div className="space-y-6">
            ...
            filteredMemoriais.map((memorial) => {
              ...
              return (
                ...
                <div className="sm:col-span-3 flex flex-col items-start gap-1.5">
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusStyle(getEffectiveStatus(memorial))}`}>
                    {getStatusLabel(getEffectiveStatus(memorial))}
                  </span>
      ...
      <Saas_NewLinkModal
        ...
        onClose={() => { setSharingMemorial(null); setManualInviteLink(null); setInviteError(null); }}
        memorial={sharingMemorial}
        shareMode={true}
        onGenerate={() => { fetchMemoriais(); setSharingMemorial(null); }}
      />
