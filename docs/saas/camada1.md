...
# MÓDULO 5 — GESTÃO INTELIGENTE DE PARTICIPANTES (ENTREVISTA)

## Problema resolvido

O painel de gerenciamento de participantes ("Enviar Link de Entrevista") precisava de regras rígidas e inteligentes para coordenar e rastrear o comportamento de envio, exclusão, reabilitação e taxa de resposta, mantendo-se em tempo real (Supabase Realtime) e assegurando bloqueios técnicos para não perder perguntas ou gerar biografias com dados incompletos.

---

## Regras dos Participantes e Novos Estados

**Limites Arquiteturais:**
* Mínimo: 1 participante (O responsável principal, que NUNCA pode ser excluído)
* Máximo: 4 participantes.

**Ciclo de Estados (`question_invites`)**
* `PENDENTE` → link enviado, familiar não acessou ainda
* `ACESSADO` → familiar abriu o link mas não concluiu
* `CONCLUIDO` → familiar respondeu e salvou todas as perguntas
* `PENDENTE_REABILITADO` → agente reabilitou o link fechado, familiar ainda não respondeu novamente. Ícone visual distinto (🔄 Link reabilitado — aguardando).

## Comportamentos Permitidos por Estado

* **PENDENTE**: [✏️ Editar] [↗ Reenviar link] [🗑️ Excluir com rateio automático]. Reabilitação não se aplica.
* **ACESSADO**: [✏️ Editar] [↗ Reenviar link] [🗑️ Excluir com aviso - pois está respondendo agora]. Reabilitação não se aplica.
* **CONCLUIDO**: [✏️ Editar nome somente] [↺ Reabilitar link] [🗑️ Excluir com geração de órfãs]. Telefone não é alterável. Reenvio não permitido (link expirado).
* **PENDENTE_REABILITADO**: [✏️ Editar nome somente] [✖ Revogar reabilitação] [🗑️ Excluir com órfãs].

## Tratamento Exclusivo para Responsável Principal
* Não pode ser excluído. 
* O link nunca expira individualmente ao concluir; expira apenas quando **TODOS** os participantes concluírem.
* Pode encaminhar links para membros pendentes diretamente de sua própria interface mobile `[📤 Encaminhar link]`.

## Regras de Reabilitação

* Somente a funerária (painel) pode reabilitar links já marcados como `CONCLUIDO`.
* Não existe reversão automática por tempo; a funerária deve cobrar o familiar via WhatsApp ou revogar a ação explicitamente.
* A revogação manual (`[✖ Revogar reabilitação]`) devolve o usuário e suas respostas intactas ao estado de `CONCLUIDO`.

**Atualização no Banco de Dados (`question_invites`)**
```sql
ALTER TABLE question_invites
  ADD COLUMN IF NOT EXISTS reabilitado_por       uuid REFERENCES organization_users(id),
  ADD COLUMN IF NOT EXISTS reabilitado_em        timestamp,
  ADD COLUMN IF NOT EXISTS total_reabilitacoes   integer DEFAULT 0;

-- Ação de Reabilitação (Agente)
...
SET
  ...
  reabilitado_por     = $1,
  reabilitado_em      = now(),
  total_reabilitacoes = total_reabilitacoes + 1
WHERE id = $2 AND status = 'concluido';

-- Ação de Revogação (Agente)
UPDATE question_invites
SET status = 'concluido', reabilitado_em = NULL
WHERE id = $1 AND status = 'pendente_reabilitado';
```

## Sistema Inteligente de Exclusão (Perguntas Órfãs vs Rateio)

A decisão sobre o que ocorre ao excluir um familiar mapeado é regida pelo andamento da entrevista:

1. **Cenário A — Ninguém Respondeu Ainda:** Ao excluir, ocorre o Rateio Automático das perguntas do excluído para os remanescentes.
2. **Cenário B — Alguns Respondem / Perguntas Parciais:** Rateio apenas das perguntas SEM respostas. As perguntas respondidas de outros familiares permanecem seguras.
3. **Cenário C — Excluir familiar que já respondeu (Órfãs):** Como as respostas físicas estão em `respostas` acopladas ao memorial, elas não são excluídas. Elas se tornam "Perguntas Órfãs", bloqueando o salvamento e a subsequente "Geração de Biografia" (congelamento obrigatório) até que a funerária defina um destino (atribuindo a um participante já existente - adicionado ao seu pacote de links sem quebrar o preexistente, ou criando um novo familiar).

**Nota de Segurança (`PENDENTE_REABILITADO`):** A exclusão de links reabilitados obriga o backend a executar uma validação física (verificação do COUNT em `respostas`) para definir se aplica o rateio de perguntas vazias ou lida como CONCLUIDO (gerando links com as perguntas órfãs para a base). Nunca perder as respostas antigas daquele grupo.

## Tempo Real e Trava de Geração

* **Estado Supabase Realtime**: Todas as visualizações contam com listen no websocket para transformar contadores, ícones de alerta e barrar ações do operador baseadas num status desatualizado. Excluir um "recente-concluído" online deve acusar "Atenção Familiar concluiu respostas."
* **Status "respostas_recebidas"**: Apenas consolidado se TODOS estiverem `CONCLUIDO` e `Zerar Perfis Órfãos`.
* O botão "Gerar Biografia" permanece estritamente desabilitado em `aguardando_respostas` e `respostas_parciais`.
